        ...
        {(items && items.length > 0) ? items.map((item, idx) => {
          return (
            <div className={styles.itemWrapper} key={`${idx}-order`}>
              <span className={styles.image}><img src={`${!title ? item.icon.split('.').map((d, idx, list) => idx === list.length - 2 ? d + '-dark' : d).join('.') : item.icon.split(':').length > 2 ? item.icon : `https://pulse.projectai.com/media/${item.icon}`}`} alt="item" /></span>

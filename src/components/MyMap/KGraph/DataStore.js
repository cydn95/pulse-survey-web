...
export default class DataStore {
  ...
  structurizeData(entities, entityId, type, currentView = null) {
    ...
    if (type === "core") {
      ...
      individuals.forEach((individual) => {
        ...
        if (!entityNodes[orgNodeId]) {
          let orgInfo = organisations.find(
            (org) => org.id === currentOrganisation
          );
          constructNode(orgInfo, orgNodeId);
          entityNodes[orgNodeId].hi =
            currentView.length > 1 || currentView[0] === "Org" ? false : true;
        ...
        orgTeamCount[orgNodeId][teamNodeId] =
          orgTeamCount[orgNodeId][teamNodeId] || 0;
    ...
    return {
      newItems: [...Object.values(entityNodes), ...Object.values(entityLinks)],
      highLevelNodes,
    };
  }
  ...
  async getCoreStructure(entityId = "ap1") {
    ...
    this.coreStructure[entityId] =
      this.coreStructure[entityId] || this.structureData;

...
function renderGraph(node, props) {
  ...
  svg.attr("width", bounds.width / 2).attr("height", bounds.height);
  ...
  root.attr(
    "transform",
    `translate(${bounds.width / 3.6}, ${bounds.height / 2})`
  );
...
const ReportCircularChart = React.forwardRef((props, ref_prop) => {
...
ReportCircularChart.defaultProps = {
  className: undefined,
  data: [],
  width: undefined,
  height: undefined,

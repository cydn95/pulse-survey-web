...
class MobileTourView extends React.Component {
  render() {
    const { tour, length, index, onBack, onNext, onSelect } = this.props;
    const divStyle = {
    ...
    return (
      <div className={classnames(styles.root)} style={divStyle}>
        ...
        <div className={styles.body}>
          <div className={styles["img-container"]} style={imgBackgroundStyle}>
          </div>
          <h1>{tour.title}</h1>
          <p>{tour.content}</p>
        </div>

...
const MyMap = (props) => {
  ...
  const defaultStakeholder = {
    ...
    lastName: "",
    email: "",
    myProjectUser: props.surveyUserId,
  };

  const myMapProjectUserList = [];
  const projectMapProjectUserList = [];
  let newProjectUser = {};

  const [screen, setScreen] = useState("list");
  const [stakeholderList, setStakeholderList] = useState([]);
  const [currentSurveyUserId, setCurrentSurveyUserId] = useState(0);
  const [currentSurveyUser, setCurrentSurveyUser] = useState("");
  const [newStakeholder, setNewStakeholder] = useState({});
  const [mapStyle, setMapStyle] = useState("my-map");
  const [viewDropDownOpen, setViewDropOpen] = useState(false);
  const [layoutDropDownOpen, setLayoutDropDownOpen] = useState(false);
  const [enableLayout, setEnableLayout] = useState(true);
  const [layout, setLayout] = usetState("Standard");
  const [viewMode, setViewMode] = useState("SH only");
  const [layoutUpdated, setLayoutUpdated] = useState(false);
  const [apList, setApList] = useState(null);
  const [esList, setEsList] = useState(null);
  const [projectApList, setProjectApList] = useState(null);
  const [projectEsList, setProjectEsList] = useState(null);
  const [teamList, setTeamList] = useState([]);
  const [userList, setUserList] = useState([]);
  const [shCategoryList, setShCategoryList] = useState([]);
  const [projectMapShCategoryList, setProjectMapShCategoryList] = useState([]);
  const [decisionMakerList, setDecisionMarkerList] = useState([]);
  const [mapSaveLoading, setMapSaveLoading] = useState(false);
  const [mapGetLoading, setMapGetLoading] = useState(false);

  const [toggleGraph, setToggleGraph] = useState(true);
  const [search, setSearch] = useState(true);

  const [searchFullHeight, setSearchFullHeight] = useState(false);

  const [myMapStakeholderList, setMyMapStakeholderList] = useState([]);
  const [projectMapStakeholderList, setProjectMapStakeholderList] = useState([]);

  const [lastAddedShCategory, setLastAddedShCategory] = useState(null);

  useEffect(() => {
    const { userId, surveyId, surveyUserId, projectId, history } = props;
    ...
    props.getKMapData(surveyUserId, userId);
    props.getProjectMapData(surveyUserId, userId);
    props.getShCategoryList(surveyId, 0);
    props.getStakeholderList(surveyUserId, surveyId);
    props.getTeamList(projectId);
    props.getAoQuestionList(surveyUserId, surveyId);
    props.getDriverList(surveyId);
    props.getSkipQuestionList();

  }, [props.userId, props.surveyId, props.surveyUserId, props.projectId, props.history, props.getKMapData, props.getProjectMapData, props.getShCategoryList, props.getStakeholderList, props.getTeamList, props.getAoQuestionList, props.getDriverList, props.getSkipQuestionList])
 ...
 */
    ...
    if (
      ...
      setDecisionMarkerList[decisionMakerList];
      setTeamList(teamList);
      setUserList(userList);
      setShCategoryList(shCategoryList);
      setProjectMapShCategoryList(projectMapShCategoryList);
      setApList(architecture);
      setEsList(individual);
      setProjectApList(projectMapArchitecture);
      setProjectEsList(projectMapIndividual);
      setScreen(list);
      setMapSaveLoading(mapSaveLoading);
      setMapGetLoading(mapGetLoading);
      setMyMapStakeholderList(myMapStakeholderList);
      setProjectMapStakeholderList(projectMapStakeholderList);
    }
  }, [props.projectId,
      props.projectTitle,
      props.stakeholderList,
      props.teamList,
      props.shCategoryList,
      props.projectMapShCategoryList,
      props.userList,
      props.kMapData,
      props.projectMapData,
      props.mapSaveLoading,
      props.mapGetLoading,
      props.aoQuestionList])
  

  const handleAddNewStakeholder = (stakeholder) => {
    const { projectId, surveyId } = props;
    props.addStakeholder(
      projectId,
      surveyId,
      stakeholder,
      callbackAddNewStakeholder
    );
  };

  const callbackAddNewStakeholder = (code) => {
  ...
  const handleAddStackholderToGraph = (data, e = {}) => {
  ...
  useEffect(() => {
    if (!newElem.individuals[0].sh_category) return;
      const newProjectUserId = newProjectUser.projectUserId;
      ...
      for (let i = 0; i < myMapProjectUserList.length; i++) {
        if (
          parseInt(myMapProjectUserList[i].projectUserId, 10) ===
          parseInt(newProjectUserId, 10) &&
          parseInt(myMapProjectUserList[i].shCategory, 10) ===
      ...
      if (!bExist) {
        myMapProjectUserList.push({
          projectUserId: newProjectUserId,
          shCategory: parseInt(newShCategory, 10),
        });
      }
      
      setLastAddedShCategory(parseInt(newShCategory, 10))

...
declare namespace KeyLines {
  ...
  interface CoreExportOpts {
    /** The required file type of the output. Default: png. */
    type: string;
    /** The maximum width and height of the output while keeping original aspect ratio. */
    fitTo?:
      | {
          width?: number;
          height?: number;
      ...
      | 'page';
    /** Specifies whether the exported result contains only the current view or the full chart. Default: 'view'. */
    extents?: 'view' | 'chart';
  }
  ...
  export interface FontDetails {
    ...
    src: string;
    /** The weight of the font encoded by this src */
    weight?: 'regular' | 'bold';
  }
  export interface FontExportOpts {
    /** A dictionary providing fonts for SVG export, indexed by font name. */
    fonts?: IdMap<FontDetails | FontDetails[]>;
  }

  export interface PNGExportOptions extends CoreExportOpts {
    type: 'png';
  }

  export interface JPEGExportOptions extends CoreExportOpts {
    type: 'jpeg';
  }

  export interface SVGExportOptions extends CoreExportOpts, FontExportOpts {
    type: 'svg';
  }
  ...
  export interface PDFExportOptions extends CoreExportOpts, FontExportOpts {
    type: 'pdf';
    /** Heading to display above the chart image */
    heading?: string;
    /** The PDFDocument object created by PDFKit to add the chart image to or options to create one  */
  ...
  interface ExportFontWarning extends ExportWarningBase, Partial<FontDetails> {
    type: 'font';
    fontFamily: string;
  }
  ...
  export interface ExportResult {
    /** The blob URL containing the location for data for the exported result. */
    url?: string;
  ...
  export interface OpenStyleOptions {
    ...
    c?: string;
    /** The height of the open rectangular combo. Set to fit all child nodes when the combo is created. */
    h?: number;
    /** Whether resize handles are shown on open combos. Default: false. */
    re?: boolean;
    /** The width of the open combo. Set to fit all child nodes when the combo is created. */
    w?: number;
  }
  ...
  export interface ShapeStyle {
    ...
    pos?: Location;
    /** Whether resize handles are shown on a shape. Default: false. */
    re?: boolean;
  ...
  export type EachItems = 'underlying' | 'toplevel' | 'all';

  export interface EachLinkOptions {
    /** The type of items to consider. */
    type: 'link';
    /** The chart items to iterate over when using combos. 'underlying': iterates over links that are not combo links. 'toplevel': iterates over links, including combos, that are not inside other combos. 'all': iterates over every link. Default: 'underlying'. */
    items?: EachItems;
  }

  export interface EachNodeOptions {
    /** The type of items to consider. */
    type: 'node';
    /** The chart items to iterate over when using combos. 'underlying': iterates over nodes that are not combo nodes. 'toplevel': iterates over nodes, including combos, that are not inside other combos. 'all': iterates over every node. Default: 'underlying'. */
    items?: EachItems;
  }

  export interface EachAllOptions {
  ...
  export type EachOptions = EachLinkOptions | EachNodeOptions | EachAllOptions;

  type ExpandLayoutName =
    | 'standard'
    | 'organic'
    | 'hierarchy'
    | 'sequential'
  ...
  export interface ExpandLayoutOptions {
    ...
    /** The options for nodes that share the same neighbours and level in the sequential layout. If `orderBy` value is specified, `stacking` only applies to nodes also sharing the same `orderBy` value. */
    stacking?: StackOptions;
    ...
    stretch?: number;
    /** The packing mode to use for the layout during expand. Not used by 'lens' or 'tweak'. Default: 'adaptive'. Default for sequential: 'aligned'. */
    packing?: 'adaptive' | 'aligned' | 'circle' | 'rectangle' | 'none';
  ...
  export interface ExpandArrange {
    /** Controls how items outside a combo (at the top chart level or within the combo's parent combo) pull back or push in to adapt to combos that changed during expand. Default: 'all'. */
    adapt?: ComboAdaptMode;
    /** Controls how nodes are arranged inside an open combo. Default: 'lens'. */
    name?: ComboArrangement;
  ...
  export interface LayoutOptions {
    ...
    Required for the 'radial' and 'hierarchy' layouts, unless the `top` option is specified. */
    level?: string;
    /** Specifies whether the layout is run in full or as a short force-directed layout that slightly adapts item positions to data changes. */
    mode?: 'full' | 'adaptive';
    /** The name of the custom property that orders the items within the same `level` in sequential layout. The items are sorted alphanumerically and in descending order. */
    orderBy?: string;
    /** The orientation of the hierarchy layout. Default: 'down'. */
    orientation?: 'down' | 'right' | 'up' | 'left';
    /** The packing mode to use for the layout. Not used by 'lens' or 'tweak'. Default: 'rectangle'. Default for sequential: 'aligned'. */
    packing?: 'adaptive' | 'aligned' | 'circle' | 'rectangle' | 'none';
    /** The spacing between nodes at each level of the sequential layout. Default: 'auto'. */
    spacing?: 'auto' | 'equal' | 'stretched';
    /** The options for nodes that share the same neighbours and level in the sequential layout. If `orderBy` value is specified, `stacking` only applies to nodes also sharing the same `orderBy` value. */
  ...
  export interface ImageAlignmentOptions {
    /** The horizontal offset as a percentage value of the item's size. The range is -50 to 50. Default: 0. */
    dx?: number;
    /** The vertical offset as a percentage value of the item's size. The range is -50 to 50. Default: 0. */
    dy?: number;
    /** The enlargement factor of the item relative to its parent. Must be greater than 0. Default: 1. */
    e?: number;
  }
  ...
  export interface SelectedNodeOptions {
    ...
    /** The style of a combo when open. */
    oc?: OpenStyleOptions;
    /** Whether resize handles are shown on a shape. */
    re?: boolean;
  ...
  export type ComboShape = 'circle' | 'rectangle';
  export interface ChartComboOptions {
    /** Sets the shape of all chart combos. Default: 'circle' */
    shape: ComboShape;
  }
  export interface ChartOptions {
    ...
    bands?: Bands;
    /** The global combo options */
    combos?: ChartComboOptions;
    ...
    /** The colour theme of the navigation and overview window controls. Default: 'light'. */
  ...
  export type ComboArrangement = 'lens' | 'concentric' | 'grid' | 'none';

  export type ComboAdaptMode = 'all' | 'inCombo' | 'none';

  export interface ComboArrangeOptions {
    /** Controls how items outside a combo (at the top chart level or within the combo's parent combo) pull back or push in to adapt to the combo that changed during arrange. Default: 'all'. */
    adapt?: ComboAdaptMode;
  ...
  export interface OpenCloseOptions {
    /** Controls how items outside a combo (at the top chart level or within the combo's parent combo) pull back or push in to adapt to the opening or closing combo. Default: 'all'. */
    adapt?: ComboAdaptMode;
    /** Whether the operation should be animated. Default: true. */
    animate?: boolean;
    /** If animated, the time the animation should take, in milliseconds. Default: 250. */
  ...
  export interface TransferOptions {
    /** Controls how items outside a combo (at the top chart level or within the combo's parent combo) pull back or push in to adapt to the combo that changed during transfer. Default: 'all'. */
    adapt?: ComboAdaptMode;
    /** Whether the transfer operation should be animated. Default: true. */
    animate?: boolean;
    /** Controls how nodes are arranged inside source and destination open combos. Default: lens. */
    arrange?: ComboArrangement;
    /** If true, the open combos will be resized to fit their contents. If false, they will not be resized, unless the contents are beyond the current boundary. Default: true. */
    resize?: boolean;
    /** If animated, the time the animation should take, in milliseconds. Default: 250. */
  ...
  export interface TimeBarOptions {
    ...
    backColour?: string;
    /** The colour theme of the time bar's control bar. Default: 'light'. */
    controlBarTheme?: 'light' | 'dark';
  ...
  export interface TimeBarPointerEventProps
    extends TimeBarPointerEventInfo,
      Pick<PointerDomEvent, Exclude<keyof PointerDomEvent, 'id'>>,
      PreventDefault {}
  ...
  export interface DraggerOptions {
    /** Drag the specified id(s) in addition to selected items. */
    alsoDrag?: string | string[];
  ...
  export type ChartDeleteHandler = () => boolean;
  ...
  export type ChartSelectionchangeHandler = () => void;
  ...
  export type ChartAllEventProps = (
  ...
  export type TimeBarAllEventProps = (
  ...
  export interface Chart {
    ...
    animateProperties(
      items:
        ...
        | LinkProperties
        | ShapeProperties
        | (NodeProperties | LinkProperties | ShapeProperties)[],
      options?: AnimatePropertiesOptions,
    ): Promise<void>;
    ...
    /** The each function allows easy iteration over all the links in the chart. The handler is called with one parameter: the current link.
     * @param options Options controlling how to iterate over the chart items.
     * @param handler The function to be called for each link in the chart. */
    each(options: EachLinkOptions, handler: (item: Link) => void): void;

    /** The each function allows easy iteration over all the nodes in the chart. The handler is called with one parameter: the current node.
     * @param options Options controlling how to iterate over the chart items.
     * @param handler The function to be called for each node in the chart. */
    each(options: EachNodeOptions, handler: (item: Node | Shape) => void): void;
    ...
    each(options: EachOptions, handler: (item: Link | Node | Shape) => void): void;
    ...
    /** The export function draws the chart into the specified image file format.
     * @param opts An options object that configures the export
     * @returns A Promise with the result of the export. */
    ...
    pan(
      direction: 'up' | 'down' | 'left' | 'right' | 'selection',
      options?: PanOptions,
    ): Promise<void>;
    ...
    zoom(
      how: 'in' | 'out' | 'one' | 'fit' | 'height' | 'selection',
      options?: ZoomOptions,
    ): Promise<void>;
  ...
  export type TimeBarChangeHandler = () => boolean;
  ...
  export type TimeBarPauseHandler = () => void;
  ...
  export interface TimeBar {
    ...
    mark(
      dts?: Date | number | TimePeriod | (Date | number | TimePeriod)[],
    ): (Date | number | TimePeriod)[];
  ...
  export interface Combo {
     ...
     * @returns A Promise. */
    close(ids: string | string[], options?: OpenCloseOptions): Promise<void>;
     ...
     * @returns A Promise. */
    combine(
      comboDefinition: ComboDefinition | ComboDefinition[],
     ...
     * @returns A Promise. */
    open(ids: string | string[], options?: OpenCloseOptions): Promise<void>;
  ...
  export interface KeyLines {
     ...
     * @param adapter A callback function which takes a URL of the CORS image as an argument and returns the value of the `crossorigin` HTML attribute as a string.
     * @returns Not used. */
    corsAdapter(adapter: (url: string) => string): void;
